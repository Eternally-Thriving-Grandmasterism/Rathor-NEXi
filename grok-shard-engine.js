...
// Mercy-gated + real Llama-3.2-1B-Instruct-onnx inference + full lattice methods + symbolic fallback
    ...
    try {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        const response = await Promise.race([
          fetch(`/${p}`),
          new Promise((_, reject) => setTimeout(() => reject(new Error(`Shard timeout: ${p}`)), 30000))
        ]);
...
export { grokShard };    utterance.pitch = skin.pitch;
    utterance.rate = skin.rate;
    utterance.volume = skin.volume;
    utterance.lang = skin.lang;
    const voices = speechSynthesis.getVoices();
